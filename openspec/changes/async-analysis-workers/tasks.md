...
- [ ] 1.1 [RED] `apps/api/src/config/env.spec.ts` (create) — `getApiEnv()` throws naming `REDIS_HOST`; out-of-range `REDIS_PORT` rejected
- [ ] 1.2 [GREEN] `apps/api/src/config/env.ts` — add `REDIS_HOST` (required) + `REDIS_PORT` to `ApiEnv`
- [ ] 1.3 `apps/api/package.json` — add `@nestjs/bullmq@11.0.4` + `bullmq@5.28.0`
- [ ] 1.4 `compose.dev.yaml` — `redis` service (`redis:7-alpine`, port `6379:6379`, `redis-cli ping` healthcheck)
...
- [ ] 2.4 `app.module.ts` — add `BullModule.forRoot({ connection: { host: env.REDIS_HOST, port: env.REDIS_PORT } })`
...
- [ ] 3.3 [GREEN] `analysis.service.ts` — inject `@InjectQueue(ANALYSIS_QUEUE_NAME) queue: Queue`; replace Phase 2 with `await this.queue.add('analyze', payload)` then return terminal; remove inline `analyze()` + Phase 3 tx + `fetchDocument`
- [ ] 3.4 `main.ts` — `REQUEST_TIMEOUT_MS = 30 * 1000`; keep keepAlive/headers
